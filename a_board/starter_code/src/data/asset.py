...
class Asset():
    ...
    # dense-surface auxiliary dense surface auxiliary: 额外采样的、更稠密的 clean 表面点云。
    # 仅在 AugmentSample.dense_clean_target_samples > 0 时填充；默认 None 零影响。
    # 必须与 sampled_vertices 处于同一 normalize 坐标系（AugmentNormalizePC 用主
    # clean 的 center/scale 同步归一化），不加噪、不参与 AugmentAddNoise。
    sampled_vertices_clean_dense: Optional[ndarray]=None
