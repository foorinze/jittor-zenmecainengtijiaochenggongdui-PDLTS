...
def compute_uniformcd_loss(pred: jt.Var, clean: jt.Var, k: int = 8,
    ...
    Args:
        ...
        k:     kNN 密度邻居数，默认 8
        dens_eps: 密度分母下限 1e-30
        force_unit_ratio: 数值等价自检开关，r 强制为 1（此时应与官方 Chamfer 相同）
        clip_lo/clip_hi: r 的裁剪区间。默认 None = 不裁剪。
            不裁剪时密度比会出现约 5 倍的膨胀，裁剪到 [0.5, 2.0] 可压到 1.7 倍。
    ...
    """
    # 注意：Jittor 的 stop_grad() 是原地生效的，会把 pred 从计算图里摘出去，
    # 实测会导致梯度被清零。所以这里直接 .numpy() 取值，不碰计算图。
