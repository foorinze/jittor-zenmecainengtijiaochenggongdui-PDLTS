...
"""auction-EMD kernel（Jittor `jt.code` 内联 CUDA）。未用于最终提交。

逐条对齐 PD-LTS 原始实现 metric/emd/emd_cuda.cu 的 6 个
        ...
        for (int k2 = 0; k2 < n; k2 += batch) {
            ...
            if (_unass_id != -1) {
                ...
                for (int k = l; k < r; k++) {
                    ...
                    float d = 3.0f - sqrtf(x2 * x2 + y2 * y2 + z2 * z2) - price_buf[k];
                    if (d > best) {
                        better = best;
                        best = d;
                        best_i = k + k2;
...
__global__ void emd_GetMax(int b, int n, int *assignment, int *bid, float *bid_increments,
    ...
    for (int i = blockIdx.x; i < b; i += gridDim.x) {
        ...
        if (assignment[i * n + j] == -1) {
            ...
            if (bid_inc - 1e-6f <= max_inc && max_inc <= bid_inc + 1e-6f) {
