...
来源: 上游 PyTorch 参考实现/models/model_light/layer.py（211 行）。
映射规则：按 PyTorch 参考实现逐层迁移，具体行为见本文件和公开实验复盘。
...
class _MLGCChannelLayerNorm(nn.Module):
    """在 Conv1d/2d 输出的 channel 轴做逐空间位置 LayerNorm。"""

    def __init__(self, channels: int, dimensions: int, eps: float = 1e-5):
        super().__init__()
        self.eps = float(eps)
        shape = (1, int(channels)) + (1,) * int(dimensions)
        self.weight = jt.ones(shape)
        self.bias = jt.zeros(shape)

    def execute(self, x: jt.Var) -> jt.Var:
        mean = x.mean(dim=1, keepdims=True)
        var = ((x - mean) ** 2).mean(dim=1, keepdims=True)
        return (x - mean) / jt.sqrt(var + self.eps) * self.weight + self.bias


class _MLGCChannelRMSNorm(nn.Module):
    """在 Conv1d/2d 输出的 channel 轴做逐空间位置 RMSNorm。"""
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        rms = jt.sqrt((x ** 2).mean(dim=1, keepdims=True) + self.eps)
...
def _make_mlgc_norm(
    ...
    """构造 MLGC 归一化层；默认 batch 路径保持历史 checkpoint 契约。"""
    mode = str(mode).lower()
    if mode == "batch":
        if dimensions == 1:
        ...
        raise ValueError(f"unsupported BatchNorm dimensions: {dimensions}")
    if mode == "group":
        max_groups = min(int(group_norm_max_groups), int(channels))
        if max_groups < 1:
            raise ValueError(
                f"group_norm_max_groups must be positive, got {group_norm_max_groups}"
            )
        groups = max_groups
    ...
    if mode == "layer":
        return _MLGCChannelLayerNorm(channels, dimensions=dimensions)
    if mode == "rms":
        return _MLGCChannelRMSNorm(channels, dimensions=dimensions)
    raise ValueError(
        f"mlgc norm mode must be batch/group/layer/rms, got {mode!r}"
    )
...
def knn_group(x: jt.Var, i: jt.Var) -> jt.Var:
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:13-30
    """
    B, N, C = x.shape
    _, M, k = i.shape
    # batch 索引 (B, 1)
...
def safe_knn(unknown: jt.Var, known: jt.Var, k: int):
    ...
    背景（density/transport loss，2026-07-25）：jittor 1.3.10 misc.py:84 的 knn 自定义
    内核按 `index` 直接寻址 `dist2/idx[j*K+i]`，没有 j>=n 守卫；其
    auto_parallel(2, block_num=256) 在 b*n 不能被 256 整除时会产生越界
    线程，向输出缓冲之后的显存野写（本仓库实测：denoise 阶段 b*n=586、
    K=1024 时约 182 线程 × 8KB ≈ 1.4MB 野写），腐蚀相邻分配（典型受害者
    是模型内 edge-conv 的 knn_idx），随后 gather 触发
    cudaErrorIllegalAddress(700)，表现为 predict 进程级崩溃且高度依赖
    进程显存布局（U4 ep4/19/29 五连崩、U3 四点与 U4 ep9 幸免）。
    本副本只加一行守卫，内核逻辑、并行方式、数值结果与原版完全一致。
...
#define K %s
    // density/transport loss 守卫：auto_parallel 尾块越界线程直接返回，禁止野写。
    if (index >= n) return;
...
def get_knn_idx(k: int, f: jt.Var, q: Optional[jt.Var] = None, offset: int = 0) -> jt.Var:
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:33-56
    """
    if q is None:
        q = f
    B, N, C = f.shape
...
class FullyConnectedLayer(nn.Module):
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:60-79
    """
...
class noiseEdgeConv(nn.Module):
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:83-116
    """
...
class PreConv(nn.Module):
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:118-142
    """

    def __init__(self, in_channel: int, out_channel: int,
                 norm_mode: str = "batch", group_norm_max_groups: int = 8):
        super().__init__()
        in_channel2 = in_channel * 2
        self.conv = nn.Sequential(
            nn.Conv2d(in_channel2, out_channel, kernel_size=1),
            _make_mlgc_norm(
                out_channel, dimensions=2, mode=norm_mode,
                group_norm_max_groups=group_norm_max_groups,
            ),
            nn.LeakyReLU(0.05),
        )
...
class EdgeConv(nn.Module):
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:144-186
    """

    def __init__(self, in_channel: int, hidden_channel: int, out_channel: int,
                 concat: bool = True, norm_mode: str = "batch",
                 group_norm_max_groups: int = 8):
        super().__init__()
        self.concat = concat
        ...
        conv_first = nn.Sequential(
            ...
            _make_mlgc_norm(
                hidden_channel, dimensions=2, mode=norm_mode,
                group_norm_max_groups=group_norm_max_groups,
            ),
            nn.LeakyReLU(0.05),
        )
...
class FeatMergeUnit(nn.Module):
    ...
    对应原仓库: 上游 PyTorch 参考实现/models/model_light/layer.py:188-210
    """
    ...
    def __init__(self, in_channel: int, hidden_channel: int, out_channel: int,
        ...
        self.convs.append(nn.Sequential(
            ...
            _make_mlgc_norm(
                hidden_channel, dimensions=1, mode=norm_mode,
                group_norm_max_groups=group_norm_max_groups,
            ),
            nn.ReLU(),
        ))
        self.convs.append(nn.Sequential(
            ...
            _make_mlgc_norm(
                out_channel, dimensions=1, mode=norm_mode,
                group_norm_max_groups=group_norm_max_groups,
            ),
            nn.ReLU(),
        ))
