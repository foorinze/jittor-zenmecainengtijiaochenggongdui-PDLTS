...
# density/transport loss E 臂：auction-EMD loss（emd_jittor kernel，G0-E 已验证）。
# 仅 components.model config 中 emd_loss_mode == "on" 时启用，替换 chamfer；
# task yaml 声明 `emd: 0.1`（对齐 历史 EMD 配方 0.1×EMD.sum()，见 losses/emd_sum.py）。
LOSS_KEY_EMD = "emd"
# density/transport loss U 臂：UniformCD 密度比对应搜索 loss（G0-U ACTIVATED）。
...
def _source_downweighted_mean(per_point, src_mask_np, alpha):
    """对 source 点（src_mask_np==1）的 per-point 损失乘 alpha，其余权 1，
    再做归一化加权平均 sum(w·loss)/sum(w)（权重归一化要求：必须按权重和归一化，
...
class PDLTSLight(ModelSpec):
    ...
    def __init__(self, model_config, transform_config):
        ...
        self.network = PDLTSLightNetwork(
            ...
            fbm_mask_mode=str(cfg.get("fbm_mask_mode", "hard")),
            fbm_soft_tail_init=float(cfg.get("fbm_soft_tail_init", 0.05)),
            fbm_topk_init_margin=float(cfg.get("fbm_topk_init_margin", 0.5)),
            nflow_module=cfg.get("nflow_module", 12),
            num_neighbors=cfg.get("num_neighbors", 32),
            mlgc_hidden=cfg.get("mlgc_hidden", 64),
            mlgc_norm_mode=str(cfg.get("mlgc_norm_mode", "batch")),
            mlgc_group_norm_max_groups=int(
                cfg.get("mlgc_group_norm_max_groups", 8)
            ),
        ...
        # density/transport loss E 臂（emd）/ U 臂（uniformcd）：替换 chamfer 的两个互斥 lane，
        # 与 dcd 两 lane 共同构成"四选一"（默认全 off = 使用默认 Chamfer 损失）。
        # 公开实验复盘记录该分支的对照性质和最终取舍。
        ...
        # density/transport loss：UniformCD 作为 基础 Chamfer 损失上的轻推附加项
        ...
        # r 裁剪区间（诊断窗口 [0.5, 2.0]）；默认 None = 不裁剪。
    ...
    # 计算后端（GPU 加速实现，2026-07-24 验证）：默认 numpy = 存量语义零变化；
...
# "vanilla_global_shared" = global [3,K] per-step resample.
    ...
    def training_step(self, batch: Dict) -> Dict:
        ...
        direction loss : 当 model_config.direction_loss_variant != "off"
        ...
        # dcd_like / dcd_official probes: when either is "on" replace
        # chamfer. Mutually exclusive (asserted at __init__).
        # density/transport loss: emd / uniformcd 分支的观测指标容器（分支未激活时保持空 dict，
        # metrics 组装点 ** 展开为零影响）。
        emd_metrics = {}
        uniformcd_metrics = {}
        if self.dcd_like_loss_mode == "on":
            ...
            # density/transport loss E 臂：EMD 替换 chamfer（历史 EMD 配方 0.1×EMD.sum()，
            ...
            # density/transport loss U 臂：UniformCD 替换 chamfer（密度 detach、
            # 对应搜索改配、L2 锚保留）。
        ...
        if self.uniformcd_nudge_mode == "on":
            ...
            # backend=jittor 可选（GPU 加速，数学式一致）。
        ...
        # (当前 N_pred == M_clean == 1024 二者相等, 但口径应与 公开目标重定向实验记录
