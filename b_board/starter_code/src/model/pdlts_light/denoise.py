...
输出点数与输入点数一致。
原仓库对应: 上游 PyTorch 参考实现/models/model_light/denoise.py:54-114.
...
def patch_denoise(
    ...
    # K 用 ceil 而不是 int/floor (实现修正): 多 1 个 patch 成本小, 覆盖率收益大.
    ...
    # density/transport loss（2026-07-25）：改用 safe_knn（带越界守卫的 jt.misc.knn
    # 副本）。此处 b*n=586 不能被 auto_parallel 的 256 整除，原版内核
    # 越界线程会向 idx 缓冲之后野写约 1.4MB，曾导致预测任务连续崩溃
