...
@dataclass
class NpyPairLazyAsset(LazyAsset):
    # load a materialized clean/noisy pair directory for training.
    # ``path`` may point to either the sample directory or its noisy.npy file.
    def load(self) -> 'Asset':
        sample_dir = self.path
        if os.path.basename(sample_dir) in ("noisy.npy", "clean.npy"):
            sample_dir = os.path.dirname(sample_dir)
        clean_path = os.path.join(sample_dir, "clean.npy")
        noisy_path = os.path.join(sample_dir, "noisy.npy")
        if not os.path.exists(clean_path) or not os.path.exists(noisy_path):
            raise FileNotFoundError(
                f"npy_pair requires clean.npy and noisy.npy under {sample_dir}"
            )
        clean = np.load(clean_path).astype(np.float64)
        noisy = np.load(noisy_path).astype(np.float64)
        if clean.shape != noisy.shape:
            raise ValueError(
                f"clean/noisy shape mismatch under {sample_dir}: "
                f"clean={clean.shape}, noisy={noisy.shape}"
            )
        asset = Asset(
            path=noisy_path,
            cls=self.cls,
            sampled_vertices=clean,
            sampled_vertices_noisy=noisy,
        )
        return asset
