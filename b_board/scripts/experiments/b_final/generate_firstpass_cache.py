#!/usr/bin/env python3
"""用 B 榜 ep149 base 生成第二遍 specialist 的 first-pass cache。
...
训练 cache 复用实际执行口径：seed_k_l1=12 / seed_k_l2=16 / alpha=80。
alpha 只控制 patch 的显存分批，不改变网络参数或 patch 集合。最终 official
推理另用 12/24；两种口径用途不同。
...
用法：
    python scripts/experiments/b_final/generate_firstpass_cache.py \\
        ...
        --base-ckpt ../checkpoints/base_ep149.pkl \\
        --chunk-size 150
"""
...
DEFAULT_BASE_CKPT = "../checkpoints/base_ep149.pkl"
DEFAULT_MODEL_CONFIG = "b_final/cache_light_alpha80"
...
def write_data_config(config_path: Path, train_root: str, chunk_datalist_starter_rel: str) -> None:
    ...
    config_path.write_text(f"""CONFIG_VERSION: b_final
...
components:
  ...
  system: b_final/firstpass_cache_predict
  model: {model_config}
...
def main() -> int:
    ...
    parser.add_argument("--run-tag-prefix", default="b_final_firstpass_cache")
